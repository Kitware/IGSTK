...
//             rotation:    m_TransformMatrix
// 
// In case the relations between all segmented fiducials are exactly the same everytime,
// we would NOT need the PtP registration. But in reality it might be possible that the
// segmentation result is slightly different, i.e. the distance between the centers of
// mass of two segmented fiducials is slightly different. This means, in reality it
// might be usefull due to segmentation errors.
// If we decide not to use PtP, the transform from PCA system to needle holder system
// could be hard coded into the software because it is ALWAYS the same. Believe me.
// See http://en.wikipedia.org/wiki/Principal_components_analysis
// The PCA -> Needle holder transform is always the same because the PCA algorithm is
// transforming the coordinate system in such a way that the 3. dimension has the
// biggest variance, i.e. it is "aligning" the data to the z axis (main axis). The
// relation between the fiducials is always the same, i.e. the main axis is always the
// same. Only the transformation changes. This means, the PCA is enough for our purpose,
// if the relations between the fiducials after segmentation stays the same.
// -Roland
// ------------------------------------------------------------------------------------
