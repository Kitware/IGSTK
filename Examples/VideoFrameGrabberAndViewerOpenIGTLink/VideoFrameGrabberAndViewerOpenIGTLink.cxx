...
VideoFrameGrabberAndViewerOpenIGTLink::VideoFrameGrabberAndViewerOpenIGTLink()
{
  ...
  */
  ...
  // The ErrorObserver is implemented in \code{VideoFrameGrabberAndViewerOpenIGTLink.h}.
  //
  // EndLatex
...
void VideoFrameGrabberAndViewerOpenIGTLink::RequestInitialize()
{
  ...
  // BeginLatex
  //
  // The \doxygen{OpenIGTLinkVideoImager} derive from \doxygen{VideoImager} and implement device
  // specific communication. For device specific implementations  see \doxygen{OpenIGTLinkVideoImager.h}
  // and \doxygen{OpenIGTLinkVideoImager.cxx}.
  //
  // EndLatex
  ...
  else
  {
    // BeginLatex
    //
    // Next we create an \doxygen{OpenIGTLinkVideoImagerTool} and set frame dimensions,
    // pixel depth, and an unique name for identification. Note these parameters
...
VideoFrameGrabberAndViewerOpenIGTLink::ErrorObserver::ErrorObserver() : m_ErrorOccured(false)
{
  //VideoImager errors
...
void
VideoFrameGrabberAndViewerOpenIGTLink::ErrorObserver::Execute(itk::Object *caller,
...
void
VideoFrameGrabberAndViewerOpenIGTLink::ErrorObserver::Execute(const itk::Object *caller,
                                      const itk::EventObject & event) throw (std::exception)
{
