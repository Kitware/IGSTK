...
void VideoFrameGrabberAndViewerCompressedDV::RequestInitialize()
{
  ...
  // The \doxygen{CompressedDVImager} derive from \doxygen{Imager} and implement device
  // specific communication. See for device specific impelementations \doxygen{CompressedDVImager.h}
  // and \doxygen{CompressedDVImager.cxx}
  //
  // EndLatex
  ...
  else
  {
    // BeginLatex
    //
    // Next we create an \doxygen{CompressedDVImagerTool} and set frame dimensions,
    ...
    CompressedDVImagerTool::Pointer imagerToolWebcam = CompressedDVImagerTool::New();
    unsigned int dims[3];
    dims[0] = 720;
    dims[1] = 576;
    dims[2] = 3;
