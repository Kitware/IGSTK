...
int Cameras::AttachAvailableCameras()
{
  ...
  int result = 0;
    ...
    std::cerr << "CalibrationDir=" << calibrationDir << std::endl;
  }
#if 0
  // Clear all previously connected camera
  vector<MCamera *>::iterator camsIterator;
  ...
  std::cerr << "Result after Cameras_Attach Available Cameras: " << result << std::endl;
  std::cerr << "Error: " << MTLastErrorString() << std::endl;
