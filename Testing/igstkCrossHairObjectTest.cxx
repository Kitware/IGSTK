/*=========================================================================
  ...
  Module:    igstkCrossHairObjectTest.cxx
  Language:  C++
  Date:      $Date$
  Version:   $Revision$
...
int igstkCrossHairObjectTest( int argc, char * argv[] )
{
  if( argc < 2 )
    {
  ...
  typedef CrossHairObjectTest::CTImageObserver 
                                                        CTImageObserverType;
  ...
  typedef igstk::CrossHairObject                ObjectType;
  typedef igstk::CrossHairRepresentation  RepresentationType;
  ...
  crossHairObjectRepresentation->SetColor(0.1,0.2,0.3);
  crossHairObjectRepresentation->SetOpacity(0.4);
  if(fabs(crossHairObjectRepresentation->GetRed()-0.1)>0.00001)
    {
    std::cerr << "GetRed() [FAILED]" << std::endl;
    TestPassed= false;
    }
  if(fabs(crossHairObjectRepresentation->GetGreen()-0.2)>0.00001)
    {
    std::cerr << "GetGreen()[FAILED]" << std::endl;
    TestPassed = false;
    }
  if(fabs(crossHairObjectRepresentation->GetBlue()-0.3)>0.00001)
    {
    std::cerr << "GetBlue() [FAILED]" << std::endl;
    TestPassed = false;
    }
  if(fabs(crossHairObjectRepresentation->GetOpacity()-0.4)>0.00001)
    {
