...
public:
  ...
  void RequestSetTransformAndParent( const Transform & transformToParent, const TParent * parent )
    ...
    if (parent == NULL)
      ...
      if (NULL == parentReferenceSystem)
        {
        igstkPushInputMacro( NullParentCoordinateSystem );
        m_StateMachine.ProcessInputs();
        return;
        }

      /** Need to store the reference system.
       *  Can't store the parent that was passed in since 
       *  we don't have it's type... 
       */
      this->m_CoordinateSystemParentToBeSet = parentReferenceSystem;
      this->m_TransformToParentToBeSet = transformToParent;

      /** State machine input invocation... */
      igstkPushInputMacro( TransformAndParent );
      m_StateMachine.ProcessInputs();

      this->m_CoordinateSystemParentToBeSet = NULL;
  ...
  void RequestComputeTransformTo(const TTarget * target)
    ...
    /** Check if the targetCoordSys is null. */
    if (NULL == targetCoordSys)
      {
      igstkPushInputMacro( NullTargetCoordinateSystem );
      m_StateMachine.ProcessInputs();
      return;
      }
    else
      {
      /** We've setup some event transduction macros 
          that depend on the event we get back from 
          RequestComputTransformTo
          */
      this->m_TargetCoordinateSystem = targetCoordSys;

      igstkPushInputMacro( ValidTargetCoordinateSystem );
      m_StateMachine.ProcessInputs();

      /** Break reference to m_TargetCoordinateSystem */
      this->m_TargetCoordinateSystem = NULL;
...
private: 
  /** Event transduction for RequestComputeTransformTo */
  igstkLoadedEventTransductionMacro( CoordinateReferenceSystemTransformTo, TransformFound );
  igstkEventTransductionMacro( CoordinateReferenceSystemTransformToNullTarget, NullTargetCoordinateSystem );
  igstkEventTransductionMacro( CoordinateReferenceSystemTransformToDisconnected, DisconnectedTargetCoordinateSystem );

  /** This private may seem redundant, but it protects against the 
    * transduction macros ending with a public declaration.
    */ 
private:
  ...
  igstkDeclareInputMacro( InternalSpatialObjectValid );
  igstkDeclareInputMacro( TransformAndParent );
  igstkDeclareInputMacro( NullParentCoordinateSystem );
  igstkDeclareInputMacro( NullParent );
  igstkDeclareInputMacro( GetTransformToParent );
  igstkDeclareInputMacro( NullTargetCoordinateSystem );
  igstkDeclareInputMacro( DisconnectedTargetCoordinateSystem );
  igstkDeclareInputMacro( TransformFound );
  igstkDeclareInputMacro( ValidTargetCoordinateSystem );

  /** States for the State Machine */
  igstkDeclareStateMacro( Initial );
  igstkDeclareStateMacro( AttachedToParent );
  igstkDeclareStateMacro( AttemptingComputeTransformTo );
  igstkDeclareStateMacro( InitialAttemptingComputeTransformTo );
  ...
  void NullParentCoordinateSystemProcessing();
  void ValidTargetCoordinateSystemProcessing();
  void NullParentProcessing();
  void GetTransformToParentProcessing();
  void TransformAndParentProcessing();
  void TransformFoundProcessing();
  void InternalSpatialObjectNullProcessing();

  /** Node of the Scene graph to which this object is attached */
  CoordinateReferenceSystem::Pointer      m_CoordinateReferenceSystem;
  Transform                               m_TransformToParentToBeSet;
  Transform                               m_TransformToParent;
  CoordinateReferenceSystem::ConstPointer m_TargetCoordinateSystem;
  CoordinateReferenceSystem::ConstPointer m_CoordinateSystemParentToBeSet;
  CoordinateReferenceSystem::ConstPointer m_CoordinateSystemParent;
