...
/** \brief This class implements the pivot calibration algorithm, locating a tools tip relative to a tracked coordinate system. 
 *
 *  We are interested in the location of a tool's tip relative to a tracked coordinate system. By rotating the tool while its tip location remains fixed we compute both the
 *  tip location relative to the tracked coordinate system and the fixed point's coordinates in the tracker's coordinate system.
 *  Every transformation \f$[R_i,\mathbf{t_i}]\f$ acquired by the tracker during the rotation yields three equations in six unknowns,
 *  \f$R_i \mathbf{t_{tip}} + \mathbf{t_i} = \mathbf{t_{fixed}}\f$, with the unknown 3x1 vectors \f$\mathbf{t_{tip}}\f$ and \f$\mathbf{t_{fixed}}\f$. 
 *  The solution is obtained in a least squares manner by acquiring multiple transformation while rotating the tool around its tip resulting in the following 
 ...
 *       \end{array}
 *     \right]\f 
 * The solution is obtained using the pseudoinverse obtained via singular value decomposition.
 */
...
public:
  ...
  /** This method adds the given transform to those used to perform the pivot calibration.
    * The method should only be invoked before calibration is performed or after it has been reset or it
    * will generate an InvalidRequestErrorEvent.
  ...
  /** This method resets the class to its initial state prior to computing a calibration 
   * (all user provided transformations are removed). */
  void RequestResetCalibration();

  /** This method performs the calibration. It generates two events: 
   *  CalibrationSuccessEvent, and CalibrationFailureEvent, denoting success or failure of the
   *  computation. Note that invoking this method prior to setting transfromations is considered valid,
   *  it will simply generate a CalibraitonFaliureEvent. In this way all sets of degenerate transformations
   *  are treated alike (empty set, data with low variability for example the same transform repeated many times).*/
   ...
   *  It generates two events: TransformModifiedEvent, and InvalidRequestErrorEvent, respectively denoting that
   *  a calibration transform is and isn't available. */
   ...
   *  It generates two events: PointEvent, and InvalidRequestErrorEvent, respectively denoting that
   *  the pivot point is and isn't available. */
  void RequestPivotPoint();

  /** This method is used to request the Root Mean Square Error (RMSE) of the linear equation system used to compute the pivot point and
   *  calibration transformation (RMSE of the vector Ax-b, see class description for more details).
   *  It generates two events: DoubleTypeEvent, and InvalidRequestErrorEvent, respectively denoting that
   *  the RMSE is and isn't available. */
   ...
   *  The default setting is PivotCalibrationAlgorithm::DEFAULT_SINGULAR_VALUE_THRESHOLD .
   *  The solution exists only if rank(A)=6 (columns are linearly independent). The rank is evaluated using
   *  SVD(A) = USV^T, after zeroing out all singular values (diagonal entries of S) that are less than the tolerance.*/
  ...
  /** Defualt threshold value under which singular values are considered to be zero. */     
...
private:
  ...
  /** Check that there are transformations, and that their variability is sufficient (rank(A) = 6)*/
  ...
  TransformContainerType m_Transforms;
             //transformations the user wants to add, because of the way the state machine works we need to first store them in 
             //a temporary variable. They will be moved to the m_Transforms container only if the state machine is in a state that
