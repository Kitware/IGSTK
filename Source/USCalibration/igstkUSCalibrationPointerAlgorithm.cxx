#include "igstkUSCalibrationPointerAlgorithm.h"
...
USCalibrationPointerAlgorithm::USCalibrationPointerAlgorithm()
{
}
...
bool
USCalibrationPointerAlgorithm::InternalComputeCalibrationProcessing()
{
  ...
  typedef lsqrRecipes::CalibratedPointerTargetUSCalibrationParametersEstimator::DataType lsqrRecipesDataType; 
  ...
  for( it=this->m_Data.begin(); it!=dataEnd; it++ ) 
  {
    ...
    dataElement.T2.setRotationMatrix(
      ...
      matrix->GetElement( 2, 0 ), matrix->GetElement( 2, 1 ), matrix->GetElement( 2, 2 ));
                //get the 2D point
    itk::Point< double, 2 > &q = ( *it ).q;
    dataElement.q[0] = q[0];
    dataElement.q[1] = q[1];
              //get the 3D point
    itk::Point< double, 3 > &p = ( *it ).p;
    dataElement.p[0] = p[0];
    dataElement.p[1] = p[1];
    dataElement.p[2] = p[2];
  ...
  else 
  {
    ...
    this->m_TransformData->RequestInitialize( transform, estimationDate,
                                              std::string("US Calibration - Calibrated Pointer"),  
