 ...
 *
 *  The InfiniTrack tracker is a firewire-based optical tracker form Atracsys sàrl 
 *  in Bottens, Switzerland. This class uses the infiniTrack library that comes with 
 *  the Tracker to communicate with the tracker camera and gather pose information 
 *  of surgical tools.
 *    
 *  Notice that the infiniTrack library will search in the application directory for 
 *  the calibration file that match the serial number of attached camera. The calibration
 *  file is identified by 0xXXXXXXXX.xml.
 *
 ...
 */
  ...
  protected:
      ...
      /** Verify if a tracker tool information is correct before attaching
          ...
          InfiniTrackTracker, location of the directory containing marker template files
    ...
    private:
      ...
      struct TrackerToolIdentification {std::string m_TrackerToolName; unsigned long m_u32GeometryID;}; 
