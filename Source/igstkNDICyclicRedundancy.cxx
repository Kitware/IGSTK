...
NDICyclicRedundancy::NDICyclicRedundancy( void )
{
...
unsigned int NDICyclicRedundancy::ComputeCyclicRedundancyOfTextData(unsigned int crc, int data)
{
...
unsigned int NDICyclicRedundancy::ComputeCyclicRedundancyOfStringInBinaryMode( const char *pszString, int nLen )
{
...
bool NDICyclicRedundancy::ValidateCyclicRedundancyOfReplyMessage(char *str, const int maxStringLength)
{
  ...
  if ( ((str[0] & 0xff) == 0xc4) && ((str[1] & 0xff) == 0xa5) ) 
  {
    ...
    computedCrc  = ComputeCyclicRedundancyOfStringInBinaryMode(str, 4);
    if ( computedCrc == containedCrc) 
    {
      ...
      computedCrc = ComputeCyclicRedundancyOfStringInBinaryMode(&str[6], (replySize-8));
  ...
  else  // TEXT format message
  {
    ...
    while((str[strLength]!= CARRIAGE_RETURN) && (strLength<maxStringLength)) //MAX_REPLY_MSG
      strLength++;

    // if measured string length is equal to maxStringLength, report error
    if (strLength>=maxStringLength)
      return false;

    str[strLength+1] = 0;

    // compute CRC of the string (FIXME: REPLACE with ComputeCyclicRedundancyOfStringInTextMode
    computedCrc = 0;
    for(int m=0; m<(strLength-4); m++)
    {
      computedCrc = ComputeCyclicRedundancyOfTextData(computedCrc,str[m]);
    }
...
unsigned int NDICyclicRedundancy::ComputeCyclicRedundancyOfStringInTextMode(const char *str, int length)
{
  unsigned int crc = 0;
  for(int m=0; m<(length);m++)
  {
    crc = ComputeCyclicRedundancyOfTextData(crc, str[m]);
  }
  return crc;
}


int NDICyclicRedundancy::AppendCyclicRedundancyTermToCommand( char *command, const unsigned int maxCommandLength )
{
  ...
  for( int m=0; m<n; m++)
  {
    ...
    crc = ComputeCyclicRedundancyOfTextData(crc,command[m]);
  } 
