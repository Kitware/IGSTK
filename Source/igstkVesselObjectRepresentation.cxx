/*=========================================================================
  ...
  Module:    igstkVesselObjectRepresentation.cxx
  Language:  C++
  Date:      $Date$
  Version:   $Revision$
...
/** Constructor */
VesselObjectRepresentation::VesselObjectRepresentation():m_StateMachine(this)
{
  // We create the ellipse spatial object
  m_VesselSpatialObject = NULL;
  this->RequestSetSpatialObject( m_VesselSpatialObject );
  
  igstkAddInputMacro( ValidVesselObject );
  igstkAddInputMacro( NullVesselObject  );

  igstkAddStateMacro( NullVesselObject     );
  igstkAddStateMacro( ValidVesselObject     );

  igstkAddTransitionMacro( NullVesselObject, NullVesselObject, 
                           NullVesselObject,  No );
  igstkAddTransitionMacro( NullVesselObject, ValidVesselObject, 
                           ValidVesselObject,  SetVesselObject );
  igstkAddTransitionMacro( ValidVesselObject, NullVesselObject, 
                           NullVesselObject,  No ); 
  igstkAddTransitionMacro( ValidVesselObject, ValidVesselObject, 
                           ValidVesselObject,  No ); 

  igstkSetInitialStateMacro( NullVesselObject );

  m_StateMachine.SetReadyToRun();
} 

/** Destructor */
VesselObjectRepresentation::~VesselObjectRepresentation()  
{
...
void VesselObjectRepresentation
::RequestSetVesselObject( const VesselObjectType * vessel )
{
  m_VesselObjectToAdd = vessel;
  if( !m_VesselObjectToAdd )
    {
    igstkPushInputMacro( NullVesselObject );
    m_StateMachine.ProcessInputs();
    }
  else
    {
    igstkPushInputMacro( ValidVesselObject );
    m_StateMachine.ProcessInputs();
    }
}

/** Null operation for a State Machine transition */
void VesselObjectRepresentation::NoProcessing()
{
}

/** Set the Cylindrical Spatial Object */
void VesselObjectRepresentation::SetVesselObjectProcessing()
{
  // We create the ellipse spatial object
  m_VesselSpatialObject = m_VesselObjectToAdd;
  this->RequestSetSpatialObject( m_VesselSpatialObject );
} 
 ...
 * object */
void VesselObjectRepresentation::UpdateRepresentationProcessing()
{
  igstkLogMacro( DEBUG, "UpdateRepresentationProcessing called ....\n");
}

/** Create the vtk Actors */
void VesselObjectRepresentation::CreateActors()
{
  // to avoid duplicates we clean the previous actors
  this->DeleteActors();
      
  if(m_VesselSpatialObject->GetNumberOfPoints() < 2)
    {
  ...
  unsigned int nPoints = m_VesselSpatialObject->GetNumberOfPoints();
  ...
  const VesselObjectType::PointType* pt = m_VesselSpatialObject->GetPoint(0); 
  ...
  for( unsigned int i=0; i < nPoints; i++ )
    {
    const VesselObjectType::PointType* pt = m_VesselSpatialObject->GetPoint(i); 
  ...
  pt = m_VesselSpatialObject->GetPoint(nPoints-1); 
...
VesselObjectRepresentation::Pointer
VesselObjectRepresentation::Copy() const
{
  Pointer newOR = VesselObjectRepresentation::New();
  newOR->SetColor(this->GetRed(),this->GetGreen(),this->GetBlue());
  newOR->SetOpacity(this->GetOpacity());
  newOR->RequestSetVesselObject(m_VesselSpatialObject);
